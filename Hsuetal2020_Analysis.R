...
rm(list=ls())
sf = read.table('Data/st_data.txt', sep='\t', header=T) # import raw data
sf.res = sf[,1:3] # subset samples information 
...
### correction zoox (algae) absorbances with blank
...
### use absorbances into trichromatic equations (Jeffrey and Humphrey, 1975)
...
afdw.lip100 = dw.lip100*(sf.res$perc_om/100) # using OM % to convert dw into afdw
...
Sys.setlocale("LC_ALL","English") # display "delta" symbol
...
comp.full = comp[complete.cases(comp),] # keep only the complete data
mat = cor(comp.full)
